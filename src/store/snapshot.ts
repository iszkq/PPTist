...
export const useSnapshotStore = defineStore('snapshot', {
  ...
  actions: {
    ...
    // 打开一个新工程时，将其作为新的撤销历史起点，避免同时保留旧工程的大型媒体快照。
    async resetSnapshotHistory() {
      const slidesStore = useSlidesStore()
      await db.snapshots.clear()
      await db.snapshots.add({
        index: slidesStore.slideIndex,
        // Dexie 不能序列化 Vue 的嵌套 Proxy，使用与常规历史快照一致的纯数据副本。
        slides: JSON.parse(JSON.stringify(toRaw(slidesStore.slides))),
      })
      this.setSnapshotCursor(0)
      this.setSnapshotLength(1)
    },
