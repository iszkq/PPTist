...
export default () => {
  ...
  const runInitialAutoAnimations = async () => {
    await nextTick()
    if (animationIndex.value !== 0 || inAnimation.value) return
  ...
  onMounted(runInitialAutoAnimations)
  watch(slideIndex, () => runInitialAutoAnimations())
