...
/**
 * HTML 动效元素。三段源码会在受 sandbox 约束的 iframe 中运行，
 * 因此可实现 CSS/JS 动效，但不能访问 PPTist 编辑器页面。
 */
export interface PPTHTMLElement extends PPTBaseElement {
  type: 'html'
  html: string
  css: string
  js: string
  widgetKind?: 'custom' | 'wheel' | 'filmCarousel' | 'particleGallery' | 'embed'
  embedUrl?: string
  wheelOptions?: string[]
  wheelRemoveWinner?: boolean
  wheelShowResult?: boolean
  wheelResultPrefix?: string
  wheelDuration?: number
  carouselImages?: string[]
  carouselDuration?: number
  carouselAutoplay?: boolean
  carouselLoop?: boolean
  particleEffect?: 'rain' | 'snow' | 'sakura' | 'stars' | 'leaves' | 'bubbles' | 'fireflies'
  particleImages?: string[]
  particleCount?: number
  particleDirection?: 'down' | 'up' | 'left' | 'right' | 'downLeft' | 'downRight' | 'random'
  particleSpeed?: number
  particlePhotoDuration?: number
  particleLoop?: boolean
  /** 在每次进入演示页时重建 iframe，从初始状态开始播放。 */
  restartOnEnter?: boolean
}


export type PPTElement = PPTTextElement | PPTImageElement | PPTShapeElement | PPTLineElement | PPTChartElement | PPTTableElement | PPTLatexElement | PPTVideoElement | PPTAudioElement | PPTHTMLElement
