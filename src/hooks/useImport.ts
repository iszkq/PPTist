...
import { extractPptxAnimations, mapPptxAnimations } from '@/utils/pptxAnimation'
import type {
  Slide,
  TableCellStyle,
  TableCell,
...
const getSourceBoundsKey = (left: number, top: number, width: number, height: number) => [left, top, width, height]
...
export default () => {
  ...
  const importPPTXFile = (files: FileList | File[], options?: { cover?: boolean; fixedViewport?: boolean }) => {
    ...
    reader.onload = async e => {
      // 先让加载遮罩完成一次渲染，避免大文件解析期间界面看起来无响应。
      await nextTick()
      await new Promise<void>(resolve => requestAnimationFrame(() => resolve()))
      let json = null
      let pptxAnimationImport = null
      try {
        const fileBuffer = e.target!.result as ArrayBuffer
        ;[json, pptxAnimationImport] = await Promise.all([
          parse(fileBuffer, {
            imageMode: 'base64',
            videoMode: 'blob',
            audioMode: 'blob',
          }),
          extractPptxAnimations(fileBuffer),
        ])
      }
      catch {
      ...
      for (const [slideIndex, item] of json.slides.entries()) {
        ...
        const slide: Slide = {
          ...
          background,
          remark: item.note || '',
          turningMode: pptxAnimationImport?.turningModes[slideIndex],
        }
        const sourceElementIdsByName = new Map<string, string[]>()
        const sourceElementIdsByBounds = new Map<string, string[]>()

        const parseElements = (elements: Element[]) => {
          const sortedElements = elements.sort((a, b) => a.order - b.order)

          for (const el of sortedElements) {
            const generatedElementStart = slide.elements.length
            let backstopSize = 1
            ...
            const sourceBoundsKey = getSourceBoundsKey(originLeft, originTop, originWidth, originHeight)
            ...
            if (slide.elements.length > generatedElementStart) sourceElementIdsByBounds.set(sourceBoundsKey, slide.elements.slice(generatedElementStart).map(element => element.id))
            const sourceElementName = 'name' in el && typeof el.name === 'string' ? el.name : ''
            if (sourceElementName && slide.elements.length > generatedElementStart) {
              sourceElementIdsByName.set(sourceElementName, slide.elements.slice(generatedElementStart).map(element => element.id))
            }
          }
        }
        parseElements([...item.elements, ...item.layoutElements])
        const sourceElementCount = pptxAnimationImport?.sourceElementCounts[slideIndex] || item.elements.length
        const sourceElementIds = slide.elements.slice(0, sourceElementCount).map(element => element.id)
        const animations = mapPptxAnimations(pptxAnimationImport?.slideAnimations[slideIndex] || [], {
          idsByIndex: sourceElementIds,
          idsByName: sourceElementIdsByName,
          idsByBounds: sourceElementIdsByBounds,
        })
        if (animations.length) slide.animations = animations
        slides.push(slide)
      }
      ...
      try {
        if (cover) {
          ...
          if (aspectRatio !== viewportRatio.value) slidesStore.setViewportRatio(aspectRatio)
          await snapshotStore.resetSnapshotHistory()
        }
        else if (isEmptySlide.value) {
          slidesStore.setSlides(slides)
          if (aspectRatio !== viewportRatio.value) slidesStore.setViewportRatio(aspectRatio)
        ...
        console.error('PPTX import commit failed:', error)
        message.error('导入后的工程保存失败，请重新尝试')
        return
      }
      finally {
        exporting.value = false
      ...
      if (pptxAnimationImport?.unsupportedCount) {
        message.warning(`已转换基础动画；${pptxAnimationImport.unsupportedCount} 个复杂动画已使用兼容效果，建议在动画面板中复核。`)
      }
    }
