...
import type { PPTLineElement, PPTElement, TableCell, TableCellStyle, PPTShapeElement, ChartType, PPTVideoElement, PPTAudioElement, PPTHTMLElement } from '@/types/slides'
...
import { HTML_WIDGET_EMPTY } from '@/configs/htmlWidget'
...
export default () => {
  ...
  /** 创建可在演示模式下运行 HTML/CSS/JS 的互动元素。 */
  const createHTMLElement = () => {
    const width = 480
    const height = 300
    const newElement: PPTHTMLElement = {
      type: 'html',
      id: nanoid(10),
      width,
      height,
